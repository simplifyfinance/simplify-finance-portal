...
import { clientIdsOn, otherDealsForSameClients, applicantIsClient, type DealLike } from '@/lib/same-clients'
      ...
      try {
        ...
        for (const id of ids) {
          const [mine, joint] = await Promise.all([
            ...
            supabase.from('deals').select(COLS).contains('fact_find_data->applicants', applicantIsClient(id)),
          ])
